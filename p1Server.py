...
def decodeResponse(response,pickup):
    nextBYTE = response[pickup: pickup + HEXBYTE]
    if(nextBYTE == ''):
        return ''
    compByte = response[pickup:pickup + 2]
    if compByte=='c0': #Compressed byte will have length 4 but start with 11
        pickup = pickup + HEXBYTE #SKIP PRIOR OCCURENCE OF NAME
        #If it is compressed we can skip the prior occurence of the name
    else:
        pickup +=2
        priorName, pickup = parseLabel(response, pickup, compByte)
    ...
    IP,pickup = parseLabel(response, pickup, RDLENGTH, "IP")
    ret=""
...
# It is easy enough to use the same method for both, but only run it once for the answer in the case of compressed
...
# (Note: We believe this functionality is useful because it is used to parse any resource record, even though not necessarily for this project without records that used compression)
...
while(data): 
    ...
    #Since we know that the header is 12 bytes total we can skip it and send in the address of the first byte we care about
    pickup = 24
    hdr = msg[:23]
